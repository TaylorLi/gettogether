...
namespace GetTogether.Mapping
{
    public static class ObjectHelper
    {
        ...
        {
            List<Mapping.PropertyMappingInfo> pmis = Mapping.ObjectHelper.GetProperties(typeof(T));
        ...
        {
            List<GetTogether.Mapping.PropertyMappingInfo> pmis = GetTogether.Mapping.ObjectHelper.GetProperties(t.GetType());
            for (int i = 0; i < pmis.Count; i++)
            {
                Type type = pmis[i].PropertyInfo.PropertyType;
                object default_value = pmis[i].PropertyInfo.GetValue(t, null);
        ...
        public static List<string> GetObjectFields(Type t)
        {
            ...
            foreach (Mapping.PropertyMappingInfo p in Mapping.ObjectHelper.GetProperties(t))
            {
            ...
            where T : class, new()
        {
            ...
            if (piFrList.Count > 0)
            {
                foreach (Mapping.PropertyMappingInfo piTo in Mapping.ObjectHelper.GetProperties(toType))
                {
