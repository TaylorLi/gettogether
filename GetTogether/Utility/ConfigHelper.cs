...
namespace GetTogether.Utility
{
    public static class ConfigHelper
    {
        ...
        {
            foreach (string s in GetTogether.Mapping.ObjectHelper.GetObjectFields(t.GetType()))
            {
                GetTogether.Mapping.ObjectHelper.SetValue<T>(t, s, GetAppSetting(s), DateHelper.DateFormat.ddMMyyyys.ToString());
            }
        }
        ...
        public static Dictionary<string, Type> GetValueTypes<T>(T t) where T : class, new()
        {
            ...
            foreach (GetTogether.Mapping.PropertyMappingInfo p in GetTogether.Mapping.ObjectHelper.GetProperties(t.GetType()))
            {
        ...
        public static List<string[]> GetConfigs<T>(T t) where T : class, new()
        {
            ...
            foreach (GetTogether.Mapping.PropertyMappingInfo p in GetTogether.Mapping.ObjectHelper.GetProperties(t.GetType()))
            {
                ret.Add(new string[] { p.DataFieldName, p.PropertyInfo.PropertyType.ToString(), GetTogether.Mapping.ObjectHelper.GetValue<T>(t, p.DataFieldName).ToString() });
            }
            return ret;
        }
