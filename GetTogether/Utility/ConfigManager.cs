...
namespace GetTogether.Utility
{
    public class ConfigManager
    {
        ...
        #region Function
        public static string GetConfigPath(SystemMode mode)
        {
            string configFolder = "Config";
            configFolder = ConfigHelper.GetAppSetting("ConfigFolder", configFolder);
            string folder = string.Concat(AppDomain.CurrentDomain.BaseDirectory, configFolder, "\\");
            if (!System.IO.Directory.Exists(folder)) System.IO.Directory.CreateDirectory(folder);
            return string.Concat(folder, mode.ToString(), ".xml");
        }

        public static bool ReadConfig<T>(ref T t, SystemMode mode) where T : class, new()
        {
        ...
        public static bool WriteConfig<T>(T t, GetTogether.Utility.ConfigManager.SystemMode mode) where T : class, new()
        {
            string path = GetConfigPath(mode);
            try
            {
        ...
        public static void InitConfig<T>(ref T t, GetTogether.Utility.ConfigManager.SystemMode mode) where T : class, new()
        {
            if (!ReadConfig<T>(ref t, mode))
            {
                Utility.ConfigHelper.SetConfig<T>(t);
                if (mode != GetTogether.Utility.ConfigManager.SystemMode.UNDEFINE)
                {
        ...
        public class ConfigInfoList : GetTogether.ObjectBase.ListBase<ConfigInfo>
        {
