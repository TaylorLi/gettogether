...
namespace GetTogether.Utility
{
    public class IP_AddressHelper
    {
        public static List<int> GetIP_Values(string ip)
        {
            ...
            List<int> ipValues = GetTogether.Utility.ListHelper.ConvertListType<string, int>(ipValuesString);
            return ipValues;
        }
            ...
            foreach (string address in ipSetting.Split(';'))
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    else if (ipInfo[i].IndexOf("-") > 0)
                    {
                        ...
                        List<int> rangeValues = GetTogether.Utility.ListHelper.ConvertListType<string, int>(rvs);
                    ...
                    else if (GetTogether.Utility.NumberHelper.ToInt(ipInfo[i], 0) == ipValues[i])
                    {
                        isAllowed = true; continue;
                    }
                    else
