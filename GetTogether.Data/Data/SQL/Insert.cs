...
namespace GetTogether.Data.SQL
{
    public class Insert
    {
        public static void BulkInsert<T, C>(C c, GetTogether.Data.ConnectionInformation ci)
            where T : GetTogether.Data.UOBase<T, C>, new()
            where C : System.Collections.Generic.ICollection<T>, new()
        {
            BulkInsert<T, C>(c, SqlBulkCopyOptions.Default, null, ci);
        }

        public static void BulkInsert<T, C>(C c, SqlBulkCopyOptions copyOptions, SqlTransaction tran,GetTogether.Data.ConnectionInformation ci)
            where T : GetTogether.Data.UOBase<T, C>, new()
            where C : System.Collections.Generic.ICollection<T>, new()
        {
            ...
            foreach (T t in c)
            {
                if (isFirst)
                {
                    ...
                    conn = ci.Connection;
                    Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Parepare Data Source"));
                }
            ...
            Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Parepare Data Source,Elapsed Milliseconds:", sw.ElapsedMilliseconds));
            sw.Stop();
            Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Collection Count:", c.Count));
            using (conn)
            {
        ...
        public static void BulkInsert(System.Data.SqlClient.SqlConnection conn, DataTable tb, string tableName, System.Data.SqlClient.SqlBulkCopyOptions copyOptions, System.Data.SqlClient.SqlTransaction tran)
        {
            if (tb != null && tb.Rows.Count != 0)
            {
                ...
                Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Table Count:", tb.Rows.Count));
                sbc.WriteToServer(tb);
                sbc.Close();
                Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Elapsed Milliseconds:", sw.ElapsedMilliseconds));
                sw.Stop();
            }
        }
        ...
        public static void BulkInsert(System.Data.SqlClient.SqlConnection conn, System.Data.SqlClient.SqlDataReader dr, string tableName, System.Data.SqlClient.SqlBulkCopyOptions copyOptions, System.Data.SqlClient.SqlTransaction tran)
        {
            if (dr != null)
            {
                ...
                sw.Start();
                sbc.DestinationTableName = tableName;
                Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Data Reader"));
                sbc.WriteToServer(dr);
                sbc.Close();
                Log.Insert(string.Concat("Batch Insert,Table Name:", tableName, ",Data Reader,Elapsed Milliseconds:", sw.ElapsedMilliseconds));
                sw.Stop();
            }
        }
    }
