...
namespace GetTogether.Data.MySQL
{
    public class SqlUtil
    {
        ...
        {
            IDataReader read = SqlUtil.ExecuteReader(conn, @"select CURRENT_TIMESTAMP()");
            using (read)
            {
        ...
        public static IDataReader ExecuteProcedureReader(IDbConnection conn, string storedProcName, MySqlParameter parameters)
        {
            return ExecuteProcedureReader(conn, storedProcName, new MySqlParameter[] { parameters });
        }
        ...
        {
            return ExecuteDataSet(GetCommandByScript(conn, cmdText));
        }
        ...
        public static IDbCommand GetIDbCommand(IDbConnection conn, string baseSql, ParameterCollection pc, bool isAsc, params object[] orderBy)
        {
            ...
            string conditions = SqlScriptHandler.GetConditionsFromParameterCollection(pc);
        ...
        {
            IDbCommand cmd = SqlUtil.GetCommandByScript(conn, SqlScriptHandler.Search.GetSelectString(tableName, pc, asc, count, orderBy));
            FillCommandByParsCollection(cmd, pc);
            return cmd;
        }
