...
namespace GetTogether.Web
{
    public class TableHelper
    {
        ...
        public static Table GenListToTable(IList list, string table_css, string header_css, string rows_css, bool chang_color, string rows_change_css, bool keep_column, int[] exist_columns, bool merge_all_columns, params int[] merge_columns)
        {
            ...
            trHeader = GetTogether.Web.TableHelper.BuiltTableRow(columns.ToArray());
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (string.IsNullOrEmpty(rows_css))
                    ret.Rows.Add(GetTogether.Web.TableHelper.BuiltTableRow(1, values.ToArray()));
                else
                {
                    if (chang_color && i % 2 == 1)
                        ret.Rows.Add(GetTogether.Web.TableHelper.BuiltTableRow(rows_change_css, values.ToArray()));
                    else
                        ret.Rows.Add(GetTogether.Web.TableHelper.BuiltTableRow(rows_css, values.ToArray()));
                }
            }
