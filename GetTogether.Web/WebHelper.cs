...
namespace GetTogether.Web
{
    public class WebHelper
    {
            ...
            foreach (string k in GetRequestKeys(method))
            {
                if (k.StartsWith(prefix))
                {
                    ...
                    if (string.IsNullOrEmpty(column))
                        continue;
                    //Mapping.ObjectHelper.SetValue(o, column, System.Web.HttpUtility.UrlDecode(System.Web.HttpContext.Current.Request[k]), GetTogether.Utility.DateHelper.GetDateString(GetTogether.Utility.DateHelper.DateFormat.ddMMyyyys));
                    GetTogether.Mapping.ObjectHelper.SetValue(o, column, System.Web.HttpContext.Current.Request[k], dateFormat);
                }
            }
            if (method != RequestPostMethod.Query && System.Web.HttpContext.Current.Request.Files != null
                && System.Web.HttpContext.Current.Request.Files.Count > 0)
            {
                foreach (string k in System.Web.HttpContext.Current.Request.Files.AllKeys)
                {
                    if (k.StartsWith(prefix))
                    {
                        string column = k.Remove(0, prefix.Length);
                        ...
                        if (string.IsNullOrEmpty(column))
                            continue;
                        GetTogether.Mapping.ObjectHelper.SetValue(o, column, System.Web.HttpContext.Current.Request.Files[k].FileName, dateFormat);
                    }
                }
            }
        }
        ...
        public static void SetValues<T>(T o, string prefix, Utility.DateHelper.DateFormat dateFormat, RequestPostMethod method)
                    where T : class, new()
        {
            SetValues(o, prefix, GetTogether.Utility.DateHelper.GetDateString(dateFormat), method);
        }

        public static void SetValues<T>(T o, string prefix)
            where T : class, new()
        {
            SetValues(o, prefix, GetTogether.Utility.DateHelper.GetDateString(GetTogether.Utility.DateHelper.DateFormat.ddMMyyyys));
        }

        public static void SetValues<T>(T o, string prefix, Utility.DateHelper.DateFormat dateFormat)
           where T : class, new()
        {
            SetValues(o, prefix, GetTogether.Utility.DateHelper.GetDateString(dateFormat));
        }

        public static List<T> GetListFromRequest<T>(string prefix)
            where T : class, new()
        {
            return GetListFromRequest<T>(prefix, GetTogether.Utility.DateHelper.DateFormat.ddMMyyyys, RequestPostMethod.Either);
        }
        ...
        public static List<T> GetListFromRequest<T>(string prefix, Utility.DateHelper.DateFormat dateFormat, RequestPostMethod method)
            where T : class, new()
        {
            return GetListFromRequest<T>(prefix, GetTogether.Utility.DateHelper.DateFormat.ddMMyyyys, method, null);
        }
